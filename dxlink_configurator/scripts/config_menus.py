...
class DGXListGen(mdc_gui.GenerateBCPU):
    """Generates a list of BCPU cards"""
    def __init__(self, parent):
        mdc_gui.GenerateBCPU.__init__(self, parent)
    ...
    def on_add(self, _):
        ...
        self.gen_list()
        for item in self.data:
            self.parent.create_add_unit(ip_ad=item, 
                                        hostname=self.com_port_txt.GetValue())
    ...
    def gen_list(self):
        ...
        try:
            ...
            start = str(self.start_card_txt.GetValue()[4:])

            finish = str(self.finish_card_txt.GetValue()[4:])
